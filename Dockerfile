...
USER root
RUN apt-get update && apt-get install -y git wget  python3-bs4 python3-pip

COPY app.py /opt/
WORKDIR /opt/
RUN git clone https://github.com/intel-iot-devkit/pneumonia-classification.git
#COPY config.json /opt/pneumonia-classification/resources/

RUN pip3 install flask matplotlib

RUN mkdir -p /opt/output && chmod 777 /opt/output
COPY default.jpeg /opt/output/input.jpeg
RUN chmod 777 /opt/output/input.jpeg
